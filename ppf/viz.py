...
class viz:
    def __init__(self, model, scene, rot, trans):
        ...
        self.dynamic = self.ax.scatter(self.transformed_cloud[0, :], self.transformed_cloud[1, :],
                                       self.transformed_cloud[2, :], marker='.', color="green")

        self.lines = []
        plt.show()

    def drawline(self, m, s):
        line, = self.ax.plot3D([self.model[0, m], self.scene[0, s]], [self.model[1, m], self.scene[1, s]],
                               [self.model[2, m], self.scene[2, s]], color='c')
